...
if ( $#ARGV == -1 ) {
   ...
   print "Uso:\tperl imap2imap-peru.pl domainmail\n";
   exit 0;
}
...
foreach my $domain (@ARGV) {
  ...
  my $sth = $dbh->prepare(qq{SELECT username,password,name FROM mailbox WHERE domain = "$domain" AND name LIKE 'noticias%'});
  $sth->execute();
  ...
  foreach my $user ( keys %users_name ) {
    my $mailbox_base_dir = "/var/spool/mailbox/migracion-peru/migrando-peru/dominios/$domain/";

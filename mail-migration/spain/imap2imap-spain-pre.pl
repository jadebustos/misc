...
my $base_dir = "/var/spool/mailbox/migracion-spain-error/";
my $mail_dir = '';
...
my $query = $dbh->prepare(qq{SELECT username,password,maildir FROM mailbox WHERE domain LIKE "%maildomain" AND username LIKE "b%"});
$query->execute();

...
my $stderr_log         = "errores-spain-v.log";
my $std_log            = "std-out-spain-v.log";
my $general_log        = "migracion-spain-v.log";
...
my $query = $dbh->prepare(qq{SELECT username,password,maildir FROM mailbox WHERE domain LIKE "%maildomain" AND username LIKE "v%"});
$query->execute();

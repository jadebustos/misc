...
my $stderr_log         = "errores-uy.log";
my $general_log        = "migracion-uy.log";
my $bbdd_log           = "bbdd-uy.log";

my $base_dir             = "/var/spool/mailbox/";
my $mail_dir             = $base_dir."uy/"; # directorio base en el que se almacenaran los dominios
my $domain               = "maildomain";


my $start_migration_date = "20070409"; # Los mensajes posteriores a este se almacenaran
                                       # como no leidos
my $mysql_server = "ip";
...
foreach my $user_dir (@users) {
  ...
  foreach (<MSG>) {
    ...
    if ( $_ =~ /^X-Folder/) {
      $folder = (split / /,$_)[1];
      # debido a la infraestructura de origen las carpetas tienen
      # anyadido el prefijo ur-
      $folder =~ s/^ur\-(.*)/$1/g;

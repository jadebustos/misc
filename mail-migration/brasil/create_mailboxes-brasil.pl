...
my $sth = $dbh->prepare(qq{SELECT username,password,maildir,quota FROM mailbox WHERE name LIKE "fabi\_%" AND domain ="$domain"});
$sth->execute();
...
open LOG,"> create_mailboxes-brasil.log";
...
system "chown -R $uid:$gid $base_dir/br/$domain";

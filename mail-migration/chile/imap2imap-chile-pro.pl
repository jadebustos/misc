...
my $imapServer = "icarus";
my $port       = 143;
my $password   = "";

# ficheros de log

my $stderr_log         = "errores-chile.log";
my $std_log            = "std-out-chile.log";
my $general_log        = "migracion-chile.log";
my $get_email_log      = "error-extraer-direccion-chile.log";
my $invalid_email_log  = "error-direccion-novalida-chile.log";
my $invalid_domain_log = "error-dominio-chile.log";
my $bbdd_log           = "bbdd-chile.log";

my $base_dir = "/var/spool/mailbox/";
my $mail_dir = $base_dir."cl/"; # directorio base en el que se almacenaran los dominios
  ...
  foreach my $dir (@domain_dirs) {
	...
	next;
      ...
      foreach my $mailmsg (@smtp_user_files) {
        ...
        elsif ( lc($this_msg_user) ne lc($user_mail) ) {
          open DIFUSERLOG,">> dif-$user_mail.log";
          print DIFUSERLOG "$user_mail - $this_msg_user - $mailmsg\n";
          close DIFUSERLOG;
          next;
        }
